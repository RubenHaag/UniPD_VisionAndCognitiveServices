...
#TODO: Is that correct? Why two output channels? We only have one feature or not? Maybe Mountain and not Mountain?
            nn.Conv2d(kernel_size = 1, in_channels=64, out_channels=n_classes),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        l3 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)(l3)
        #Cropping of second level
        dX = l2.size()[2] - l3.size()[2]
        dY = l2.size()[3] - l3.size()[3]
        print(l2.size(), l3.size())
        ...
        l2 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)(l2)
